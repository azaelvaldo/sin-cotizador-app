...
function getAlerts(): Alert[] {
  ...
  try {
    const parsed = JSON.parse(raw) as Array<
      Omit<Alert, 'createdAt'> & { createdAt: string | Date }
    >;
  ...
  const createTestAlert = () => {
    const messages = {
      ...
      success: {
        ...
        message: 'Nueva cotización ha sido aprobada exitosamente',
      },
      warning: {
        title: 'Cotización pendiente',
        message: 'Hay cotizaciones que requieren revisión',
      },
      error: {
        title: 'Error en el sistema',
        message: 'Se detectó un problema que requiere atención',
      },
    } as const;
                      ...
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h4
                              className={`font-medium ${alert.read ? 'text-gray-600' : 'text-gray-900'}`}
                            >
                              {alert.title}
                            </h4>
                            {!alert.read && (
                          ...
                          <p
                            className={`text-sm mb-2 ${alert.read ? 'text-gray-500' : 'text-gray-700'}`}
                          >
                            {alert.message}
                          </p>
